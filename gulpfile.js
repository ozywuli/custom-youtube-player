...
gulp.task('build:js', () => {
  return browserify({entries: path.join('src', 'customYoutubePlayer.js'), debug: false, standalone: 'CustomYoutubePlayer'})
        .transform("babelify", {presets: ["es2015"]})
        .bundle()
          .on('error', showError)
        .pipe(source('customYoutubePlayer.js'))
        .pipe(buffer())
        .pipe(gulp.dest('dist'))
        .pipe(rename('customYoutubePlayer.min.js'))
